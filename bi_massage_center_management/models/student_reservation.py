...
class StudentReservation(models.Model):
    ...
    _description = "Client reservation"
    ...
    level_id = fields.Many2one('res.partner', string="Message Center", domain=[
        ('is_sport', '=', True)])
    ...
    def _compute_name(self):
        for record in self:
            coach_name = record.trainer_id.name if record.trainer_id else "No Therapist"
            student_name = record.student_id.name if record.student_id else "No Client"
    ...
    def action_change_details(self):
        ...
        view_id = self.env.ref('bi_massage_center_management.reservation_change_wizard_form').id
        ...
        for record in self:
            if record.trainer_id and record.start_date and record.end_date:
                ...
                if overlapping_reservations:
                    ...
                    start_date_local = record.start_date.replace(tzinfo=timezone('UTC')).astimezone(user_timezone)
                    end_date_local = record.end_date.replace(tzinfo=timezone('UTC')).astimezone(user_timezone)
