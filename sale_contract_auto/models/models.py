...
class SaleOrder(models.Model):
    ...
    contract_count = fields.Integer(string='Contract Count', compute='_compute_contract_count', tracking=True)
    contract_template_id = fields.Many2one('contract.template', string='Contract Template', tracking=True)
    contract_id = fields.Many2one('sale.contract', string='Contract', compute='_compute_contract', store=False, tracking=True)

    def _compute_contract(self):
        for order in self:
            contract = self.env['sale.contract'].search([('sale_order_id', '=', order.id)], limit=1)
...
class ContractTemplate(models.Model):
    ...
    content = fields.Html(string='Agreement Terms', tracking=True)
